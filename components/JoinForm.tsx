...
export function JoinForm({ email }: { email: string }) {
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const body = [
      ...
      `Programme of interest: ${String(data.get('programme') ?? '')}`,
      '',
  ...
  return (
    <form onSubmit={handleSubmit} className="measure">
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <div className="sm:col-span-2">
          <label htmlFor="join-programme" className={label}>
            Which programme interests you
          </label>
          <select id="join-programme" name="programme" defaultValue="" className={field}>
            <option value="">No preference</option>
            <option value="Town Service Day">Town Service Day</option>
            <option value="Scholarship">The Scholarship</option>
            <option value="Standing Donor Register">Standing Donor Register</option>
            <option value="Youth League">Youth League</option>
            <option value="Elders Programme">Elders Programme</option>
          </select>
        </div>
        <div className="sm:col-span-2">
      ...
      <button
        type="submit"
        className="group mt-7 inline-flex items-center gap-2 bg-accent px-6 py-3.5 font-semibold text-page transition-colors hover:bg-accent-strong"
      >
        Send enquiry
